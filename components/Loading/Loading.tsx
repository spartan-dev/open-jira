...
const Loading: FunctionComponent<LoadingProps> = ({ children }): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (!loadingRoot || !elementRef.current)
      throw new Error("HTML root element not present in document")

    loadingRoot.appendChild(elementRef.current)

    return () => {
      if (loadingRoot && elementRef.current) loadingRoot.removeChild(elementRef.current)
    }
  }, [])

  return createPortal(
    <div className={styles.loading}>{children ? children : <p>Loading...</p>}</div>,
    elementRef.current
  )
}
